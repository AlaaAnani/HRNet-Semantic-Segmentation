...
class BaseDataset(data.Dataset):
    ...
    def inference(self, config, model, image, flip=False, unscaled=False, device='cuda'):
    ...
    def multi_scale_inference_noisybatch(self, config, model, n, sigma, 
                                         image_np, 
                                         normalize=True, 
                                         scales=[1], 
                                         flip=False, 
        ...
        for scale in scales:
            ...
            if scale <= 1.0:
                preds = self.inference(config, model, torch.from_numpy(new_images_s), flip, unscaled=unscaled)
                if not unscaled:
